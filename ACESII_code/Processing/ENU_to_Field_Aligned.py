...
def ENU_to_Field_Aligned(wRocket, wFile, rocketFolderPath, justPrintFileNames, wflyer):
    ...
    fileoutName = input_names[wFile].replace('ENU_','Field_Aligned_')
    ...
    if justPrintFileNames:
        ...
        compNames = [key for key, val in data_dict.items() if key.lower() not in ['de_mag', 'db_mag', 'epoch']]
        ...
        if outputData:
            ...
            data_for_output_FA = np.array([[data_for_output[i][0], data_for_output[i][1], data_for_output[i][2], np.linalg.norm(data_for_output[i])] for i in range(len(data_for_output))])
            ...
            outputCDFdata(outputPath, data_dict_output, outputModelData, globalAttrsMod, wInstr)
