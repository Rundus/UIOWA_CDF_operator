...
wRocket = 5
modifier = ''
inputPath_modifier = '\science\SSAcomponents_E' # e.g. 'L1' or 'L1'. It's the name of the broader input folder
outputPath_modifier_dE = 'science\deltaE'
...
def mSSA_to_deltaE(wRocket, wFile, rocketFolderPath, justPrintFileNames, wflyer):
    ...
    if justPrintFileNames:
        ...
        print('\n')
        print(color.UNDERLINE + f'mSSA_to_deltaE Data' + color.END)
        ...
        # --- output file name ---
        fileoutName_dB = f'ACESII_{rocketID}_l2_E_Field_dE_{coordinates}_WL{SSA_window_Size}_{TimeWindow}'

        # --- get the data from the SSAcomponet file ---
        prgMsg(f'Loading data from {inputPath_modifier} E_Field Files')
        data_dict_SSAcomps = loadDictFromFile(inputFiles[wSSAFile],{},reduceData=False,targetTimes=[])
        Done(start_time)

        # prepare data for further processing
        comps = ['E_East', 'E_North', 'E_Up'] if coordinates in ['ENU','kenton'] else ['E_e', 'E_p', 'E_r']
        ...
        E_SSA = [data_dict_SSAcomps[compNames[0]][0], data_dict_SSAcomps[compNames[1]][0], data_dict_SSAcomps[compNames[2]][0]]
        ...
        data_for_output = []
        newComps = ['dE_East', 'dE_North', 'dE_Up'] if wUseData in [0,1] else ['dE_e', 'dE_p', 'dE_r']
        ...
        if outputData_dE:
            prgMsg('Creating dE output file')

            # create the output data_dict
            data_dict = deepcopy(data_dict_SSAcomps)
            newComps = ['dE_East', 'dE_North', 'dE_Up', 'dE_Mag']
            ...
            outputPath = f'{rocketFolderPath}{outputPath_modifier_dE}\{fliers[wflyer]}\\{fileoutName_dB}.cdf'

            outputCDFdata(outputPath, data_dict, outputModelData, globalAttrsMod, 'E_Field')
...
if len(glob(f'{rocketFolderPath}{inputPath_modifier}\{fliers[wflyer]}\*.cdf')) == 0:
    ...
    mSSA_to_deltaE(wRocket, 0, rocketFolderPath, justPrintFileNames,wflyer)
