...
wUseData = 0
# --- --- --- REDUCE DATA --- --- ---
reduceDataSet = True # I DON'T CARE ABOUT LF BEFORE 17:24 or HF before 17:22
targetTimes = [
            [dt.datetime(2022, 11, 20, 17, 22, 00, 000000), dt.datetime(2022, 11, 20, 17, 30, 00, 000000)],
...
generate_these_files = [4] # of the number of files in "breakIntoThisManyPieces", only generate those in generate_these_files
...
def L2_mag_to_mSSA(wRocket, wFile, rocketFolderPath, justPrintFileNames, wflyer):
    ...
    if justPrintFileNames:
        ...
        data_dict_mag = loadDictFromFile(inputFiles[wFile], reduceData=True,targetTimes=targetTimes[wRocket-4])
        ...
        if SECTION_SSA:
            ...
            if STICH_DATASET:
                ...
                if generate_these_files != []:
                    rangeLimits = [rangeLimits[num] for num in generate_these_files]
            else:
                rangeLimits = [[]]
            ...
            for k, limits in enumerate(rangeLimits):
                ...
                outputPathSSA = f'{rocketFolderPath}\\L3\\SSAcomponents_B\\{fliers[wflyer]}\\Stitching\\ACESII_{rocketID}_RingCore_SSAcomponents_{searchMod}_WL{SSA_window_Size}_subset_{generate_these_files[k]}.cdf'
