...
# --- LOAD IN THE DATA ---
# --- --- --- --- --- ---
print(color.UNDERLINE + f'Plot3_HighFlyer_waveInfo' + color.END)
prgMsg('Loading Data')
...
for i in range(len(targetVar)):
    ...
    data_dict_EISCAT_interp = InterpolateDataDict(InputDataDict=data_dict_EISCAT,InputEpochArray=data_dict_EISCAT['Epoch'][0],targetEpochArray=data_dict_deltaB['Epoch'][0],wKeys=[])
...
#######################################
def PlotExtra_HighFlyer_waveInfo(targetVar,dict_sets):
    ...
    for i in range(len(targetVar)):
        ...
        B0 = 1E-9 * data_dicts[2]['Bmag'][0]
        ni = (cm_to_m ** 3) * data_dicts[1]['ni'][0]
        ...
        rhoCalc = (ni*data_dicts[3]['Ion_Comp'][0]*((IonMasses[4] + IonMasses[5] + IonMasses[7])/3) + IonMasses[1]*ni*data_dicts[3]['Op_Comp'][0])
        ...
        # calculate Estimated Poynting Flux
        S_est = VA_t*(dBperp**2)/ (u0)
        ...
        data_dict_FFT['dBperp'].append(dBperp)
        ...
        data_dict_FFT['S_est'].append(S_est)
    ...
    waveSetLabels = [['$\delta B_{e}$', '$\delta B_{r}$'],
                     ['$\delta B_{e}$', '$\delta B_{r}$']]

    spectraFreqs = [data_dict_FFT['xf_deltaB'], data_dict_FFT['xf_deltaB']]

    PoyntingFluxs = [data_dict_FFT['S_est'], data_dict_FFT['S_est']]

    spectraData = [  [data_dict_FFT['Be_fft'], data_dict_FFT['Br_fft']],
                     [data_dict_FFT['Be_fft'], data_dict_FFT['Br_fft']]
                     ]

    spectraLabels = [['$\delta B_{e}$ [nT]', '$\delta B_{r}$ [nT]'],
                     ['$\delta B_{e}$ [nT]', '$\delta B_{r}$ [nT]']]
    ...
    for wKeySet in kSets:
        ...
        S_estFluxs = PoyntingFluxs[wKeySet]
        ...
        for i in range(len(targetVar)):
            ...
            ax[0][i].plot(data_dicts[1]['ILat'][0], mod*data_dicts[0][wWaveSetKeys[1]][0], color=plotColors[1], linewidth=plotLineWidth,label=f"{wWaveSetLabels[1]} [nT]")
            ax[0][i].set_xmargin(0)
        ...
        plt.tight_layout()
        fileOutName = "PlotExtra_WaveAnalysis_BeBr.png"
        outputPath = rf'C:\Users\cfelt\OneDrive\Desktop\Papers\ACESII_Alfven_Observations\PlotExtra\{fileOutName}'
...
# --- EXECUTE ---
#################
PlotExtra_HighFlyer_waveInfo(targetVar,dict_sets)
