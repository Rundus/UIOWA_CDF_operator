...
### DATA TOGGLES ###
targetTimes = [dt.datetime(2022, 11, 20, 17, 25, 1, 400000), dt.datetime(2022, 11, 20, 17, 25, 2, 000000)]
#             100,110,120,130,140,150,160,170
wPitchBins = [11, 12, 13, 14, 15, 16, 17, 18] # Index of the pitch bins to use in the figure
Engylimit = 17 # corresponds to the highest energy used
...
VperpI = np.zeros(shape=len(data_dict_counts_high['Epoch'][0])*len(wPitchBins)*len(Energy))
VparaI = np.zeros(shape=len(data_dict_counts_high['Epoch'][0])*len(wPitchBins)*len(Energy))
...
for indx, alt in enumerate(simAlt):
    simEpara[indx] = calcEpara(EmagI, pitchI, np.array([alt for i in range(len(VparaI))]))
...
# print(Epara_bins)
Epara_bins = Energy[:Engylimit:-1]
Epara_bins = [1 + 40*i for i in range(25)]

dataHistogram = np.zeros(shape=(len(simAlt), len(Epara_bins)-1))

for indx, alt in tqdm(enumerate(simAlt)):

    # expand the Vpara variable for every count detected at a specific velocity
    EparaExpanded = list(itertools.chain(*[[simEpara[indx] for i in range(int(val))] for val in simCounts[indx]]))

    # Make a histogram of the VparaExpanded
    hist, bins = np.histogram(EparaExpanded, Epara_bins)

    dataHistogram[indx] = hist
...
# the plot
cmap = ax.pcolormesh(Epara_bins[:-1],simAlt,dataHistogram/dataHistogram.max(),cmap=cmap,vmin=0,vmax=1)
...
cbar.set_label('Counts', fontsize=labelsFontSize)
