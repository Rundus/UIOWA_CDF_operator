...
dispersiveRegionTargetTime = [dt.datetime(2022,11,20,17,24,57,600000),
                              dt.datetime(2022,11,20,17,25,1,500000)]
...
wSTEBtoPlot = [2,3,4,5] # STEB number NOT index. Don't -1
...
wPitchs_to_plot = [2] # decide which pitch angles to get the peak energy for
countsMask = 5
...
# x = data_dict_counts_high['Epoch'][0][IndexLow:IndexHigh]
rktTime_counts = EpochTo_T0_Rocket(InputEpoch=data_dict_counts_high['Epoch'][0][IndexLow:IndexHigh], T0=data_dict_counts_high['Epoch'][0][0])
y = data_dict_counts_high['Energy'][0]
X, Y = np.meshgrid(rktTime_counts, y)
Z = deepcopy(X)

# populate the new data
for tme in range(len(rktTime_counts)):
    for engy in range(len(y)):
...
ax[0].set_ylabel('Energy [eV]',fontsize=labels_FontSize, labelpad=labels_Padding+15)
...
rktTime_deltaB = EpochTo_T0_Rocket(InputEpoch=data_dict_deltaB['Epoch'][0], T0=data_dict_counts_high['Epoch'][0][0])
ax[1].plot(rktTime_deltaB, PoyntingScale*S_est, plotColors[2],linewidth=plot_LineWidth,zorder=2)
ax[1].set_ylabel('$\delta$ S$_{p}$ [ergs/cm$^{2}$s]',fontsize=labels_FontSize,labelpad=labels_Padding+15)
...
STEBtimes_rkt = [ [(pycdf.lib.datetime_to_tt2000(tme[0]) - pycdf.lib.datetime_to_tt2000(data_dict_counts_high['Epoch'][0][0]))/1E9,(pycdf.lib.datetime_to_tt2000(tme[1]) - pycdf.lib.datetime_to_tt2000(data_dict_counts_high['Epoch'][0][0]))/1E9] for tme in STEBtimes]
...
for k,tme in enumerate(STEBtimes_rkt):
    ...
    ax[1].axvspan(rktTime_deltaB[lowIdx], rktTime_deltaB[highIdx], color=regionColors[k], alpha=0.25,zorder=1)
...
for idx,ptchVal in enumerate(wPitchs_to_plot):
    ...
    for tme in STEBtimes_rkt:
        ...
        STEBdata = data_dict_counts_high['eepaa'][0][lowIdx:highIdx]
        ...
        for timeIdx in range(len(STEBdata)):
            ...
            countsArray = STEBdata[timeIdx,ptchVal,Energy_yLimit_Idx:]
            peakE_Idx = next((i for i, x in enumerate(countsArray) if x), None)
    ...
    # plot the results
    ax[2].plot(avgTimes, peakEnergy, color=plotColors[idx], label=rf'$\alpha = {Pitch[ptchVal]}^\circ$', marker='.', ms=plot_MarkerSize)
    ax[3].plot(avgTimes, peakCounts, color=plotColors[idx], label=rf'$\alpha = {Pitch[ptchVal]}^\circ$', marker='.', ms=plot_MarkerSize)


ax[2].set_ylabel('Peak Energy [eV]',fontsize=labels_FontSize,labelpad=labels_Padding+3)
ax[2].legend(fontsize=legend_FontSize)
ax[3].set_ylabel('Peak Counts',fontsize=labels_FontSize,labelpad=labels_Padding+15)
...
# Histogram Colorbar
caxH = fig.add_axes([0.93, 0.8, 0.025, 0.158])
cbar_hist = plt.colorbar(mpl.cm.ScalarMappable(norm=histNorm, cmap=cmap_hist), cax=caxH, drawedges=False)
cbar_hist.set_label(f'Median Pitch Angle', fontsize=labels_FontSize+8, rotation=270)
...
# output the figure
plt.subplots_adjust(left=0.15, bottom=0.055, right=0.92, top=0.99, wspace=None, hspace=0.08)
fileOutName = rf'C:\Users\cfelt\OneDrive\Desktop\Papers\ACESII_Alfven_Observations\Plot7\\Plot7_KeyObservations.png'
