...
def Distribution_Function(wRocket, wFile, rocketFolderPath, justPrintFileNames, wflyer):
    ...
    DistFuncFiles = glob(f'{scienceFolderPath}DistFunc\{fliers[wflyer]}\*.cdf')

    L2_names = [file.replace(f'{rocketFolderPath}L2\{fliers[wflyer]}\\', '') for file in L2Files]
    DistFunc_names = [file.replace(f'{scienceFolderPath}DistFunc\{fliers[wflyer]}\\', '') for file in DistFuncFiles]
    ...
    if justPrintFileNames:
        ...
        outputPath = f'{scienceFolderPath}DistFunc\{fliers[wflyer]}\\{fileoutName}'
